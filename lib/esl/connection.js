...
var Connection = exports.Connection = function(host, port, password, cb) {
    events.EventEmitter.call(this);

    this.readyCb = cb;

    if(arguments.length === 3) { //"Inbound" connection (going into FSW)
	...
	this.authed = false;
        ...
        this.socket = net.connect({
            port: port,
            host: host
        }, utile.proxy(this._onConnection, this));
    }
    else if(arguments.length === 1) { //"Outbound" connection (coming from FSW)
        this._inbound = false;
        this.fd = host;
    }
    else {
...
Connection.prototype.getInfo = function() {
    ...
    //return ESLevent that we got from CHANNEL_DATA response
};
...
Connection.prototype.send = function(command, args, body) {
...
//NOTE: This is a wrapper around send, that uses an ESLevent for the data
...
//  that is all it does. It does not block as the description says, nor does
//  it queue events. Node has a better Event system than this, use it.
//  It does timeout after the specified ms however.
Connection.prototype.recvEventTimed = function(ms, cb) {
    ...
    this.once('esl::event', fn);
};

//See the event socket filter command (http://wiki.freeswitch.org/wiki/Event_Socket#filter).
Connection.prototype.filter = function(header, value) {
    this.send('filter ' + header + ' ' + value);
};
...
Connection.prototype.events = function(type, events) {
    if(['plain','xml','json'].indexOf(type) == -1)
	type = 'plain';

    events = events || 'ALL';

    this.send('event ' + type + ' ' + events);
};
...
//
//NOTE: Sync? In NODE?! Idk about that...
 ...
 ** Private helpers
 **********************/
Connection.prototype._onConnection = function() {
    //initialize parser
    ...
    this.once('esl::auth::request', utile.proxy(this.auth, this));
};
...
Connection.prototype._onEvent = function(event, headers, body) {
    ...
    case 'text/event-plain':
        ...
        event = new esl.Event(extra);
        emit = extra['Event-Name'];
        break;

    default:
        emit = headers['Content-Type'];
    }
