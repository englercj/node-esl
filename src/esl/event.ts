...
export class Event {
    ...
    constructor(type?: any, subclass?: string) {
        this.headers = [];
    ...
    serialize = function (format?: string) {
        format = format || 'plain';
        ...
        switch (format) {
            case 'json':
                ...
                */
                ...
                data = this.headers.reduce(function (d: any, header: any) {
    ...
    _findHeader(name: string) {
        var i = this._findHeaderIndex(name);

        return (i === null ? null : this.headers[i]);
    }

    static PRIORITY: {[index: string]: EventPriority} = {
        LOW: 'LOW',
        NORMAL: 'NORMAL',
        HIGH: 'HIGH',
        INVALID: 'INVALID'
...
export type EventPriority = 'NORMAL' | 'LOW' | 'HIGH' | 'INVALID';