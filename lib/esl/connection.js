...
var Connection = exports.Connection = function() {
	...
	this.once('esl::channel::data', function(evt) {
	    self.emit('ready');
	...
	*/
    ...
    //Make background events trackable and unique
    this.on('esl::event::*', function(evt) {
	var uuid = evt.getHeader('Job-UUID') || evt.getHeader('Unique-ID') || evt.getHeader('Core-UUID');

	if(uuid) self.emit(this.event + '::' + uuid, evt);
    });
};
...
Connection.prototype.sendRecv = function(command, args, cb) {
    ...
    this.once('esl::command::reply', cb);
...
Connection.prototype.bgapi = function(command, args, jobid, cb) {
    ...
    var self = this;
    self.sendRecv('bgapi ' + command + args, function(evt) {
...
//self.fsw.bgapi('originate', 'sofia/internal/8507585138@proxy.patlive.local &echo', function(e) {
...
Connection.prototype._onEvent = function(event, headers, body) {
    ...
    case 'text/event-xml':
        emit = 'esl::event::' + event.getHeader('Event-Name');
	break;
    ...
    console.log('EMIT:', emit, headers['Content-Type']);
